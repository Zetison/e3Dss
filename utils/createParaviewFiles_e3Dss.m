...
for m = 1:M
    ...
    switch layer{m}.media
        case 'fluid'
            ...
            if m == 1
                [visElements{m}, nodes{m}] = meshRectangleWcircHole([-1.3*R_a, -R_a],[1.3*R_a, R_a],R_i,round(extraPts*1.3*R_a));
                nodes{m} = [nodes{m}, zeros(size(nodes{m},1),1)];
            elseif R_i == 0
                [visElements{m}, nodes{m}] = mesh2DDisk(R_o,round(1.18*extraPts*2*R_o));
                nodes{m} = [nodes{m}, zeros(size(nodes{m},1),1)];
            else
                [visElements{m}, nodes{m}] = mesh2DDonut(R_o,R_i,round(1.18*extraPts*2*R_o));
                nodes{m} = [nodes{m}, zeros(size(nodes{m},1),1)];
            end
        case 'solid'

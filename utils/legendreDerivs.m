...
elseif n == 1
    ...
    temp = P(2,:);
    P(2,:) = 1/n*((2*n-1)*x.*P(2,:) - (n-1)*P(1,:));
    P(1,:) = temp;
end
if nargin >= 4
    if n == 1
        dP = [zeros(size(x)); ones(size(x))];
        if isa(x,'sym')
            dP = vpa(dP);
        end
    elseif n == 2
    ...
    elseif n > 2
        temp = dP(2,:);
        dP(2,:) = 1/n*((2*n-1)*(P(1,:) + x.*dP(2,:)) - (n-1)*dP(1,:));
        dP(1,:) = temp;
    end
end
if nargin >= 5
    if n == 2
        d2P = [zeros(size(x)); 3*ones(size(x))];
        if isa(x,'sym')
            d2P = vpa(d2P);
        end
    elseif n == 3
    ...
    elseif n > 3
        temp = d2P(2,:);
        d2P(2,:) = 1/n*((2*n-1)*(2*dP(1,:) + x.*d2P(2,:)) - (n-1)*d2P(1,:));
        d2P(1,:) = temp;
    end
end
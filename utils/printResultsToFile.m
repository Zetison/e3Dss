function printResultsToFile(filename, newOptions)
...
switch options.format  
    ...
    case 'BeTSSi'
        ...
        if isfield(task,'institution')
            fprintf(fid, '%s\n', task.institution); % Line 2: institution
        else
            fprintf(fid, 'NTNU_FFI\n'); % Line 2: institution
        end
        if externalProvider
            fprintf(fid, '%s\n', task.comments); % Line 3: Comments, Computer specs. and calculation times   

...
if 0
    ...
    X = layer{1}.R*[sin(theta_arr), zeros(size(theta_arr)), cos(theta_arr)]; % Evaluate physical location of plotting points;
    layer{1}.X = X;
    layer{1}.calc_p = 1;
    layer{2}.X = X;
    ...
    figure(12)
    plot(theta_arr, abs(layer{2}.sigma_s{1}+layer{1}.p),'DisplayName','Exact')
%     plot(theta_arr, real(layer{2}.sigma_s{1}),'DisplayName','Exact')
    ...
    legend('show');
%     real(layer{2}.sigma_s{1})

    figure(42)
    semilogy(0:(size(relTermMaxArr,2)-1),relTermMaxArr.')
    xlabel('n')
    hold off
    legend('show');
end

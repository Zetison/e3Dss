...
if ~isfile('miscellaneous/U_pol_double.mat') || ~isfile('miscellaneous/U_pol_mp.mat')
    for prec = {'double', 'mp'}
        switch prec{1}
            case 'double'
                i_max = 64;
        ...
        U_pol = cell(i_max,1);
        u_k = zeros(i_max,1,class(i_max));
        v_k = zeros(i_max,1,class(i_max));
        for i = 1:i_max
